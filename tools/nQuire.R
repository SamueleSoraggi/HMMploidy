...
#function for gmm. if unifNoise is TRUE, then the first component is Uniform(0,1) noise, and the other gaussian components. k=number of gaussians. alpha0 must includes a proportion  
...
totLth=as.numeric(args[4])
    ...
    for(NINT in 0:(ceiling(totLth/lth) - 1)){
        ...
        cat(chosen,"\n",file=paste(baseName,lth,"gmmu",sep="."),sep="\t",append=(NINT>0))
        cat(baseName," done\n",sep="")
    }
